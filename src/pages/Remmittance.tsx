...
const Remmittance = () => {
  ...
  const CountryListSorted = CountryListData?.data?.data
    ?.slice()
  ...
  const savedEntity = localStorage.getItem("savedEntity")
    ? JSON.parse(localStorage.getItem("savedEntity") || "")
    : [];
  ...
  const documentTypeArray: { value: string; label: string }[] = [
    ...
    {
      value: "business-transaction-documents",
      label: "Business Transaction Documents",
    },
    {
      value: "loan-agreements-repayment-proof",
      label: "Loan Agreements and Repayment Proof",
    },
  ...
  const summaryList = [
    ...
    },
    {
      ...
      value: (
        <p>
          {fromAmount} {selectFromCurrency?.currency?.symbol}
        </p>
      ),
    },
    {
      ...
      value: (
        <p className="font-normal text-orange-300">
          {toAmount} {selectToCurrency?.currency?.symbol}
        </p>
      ),
    },
  ];
  ...
  const handleCountrySearch = (value: string) => {
    const filteredOptions =
      CountryListSorted &&
      CountryListSorted.filter((item: any) =>
  ...
  const handleSelectFromCurrency = (id: string) => {
    const searchSelectFromCurrency = CurrencyListArray.find(
      (item: any) => item.id === id
    );
  ...
  const handleSelectToCurrency = (id: string) => {
    const searchSelectToCurrency = CurrencyListArray.find(
      (item: any) => item.id === id
    );
  ...
  const onUserSelect = (value: any) => {
    const selectedOption = Users?.find((item: any) => item.id == value);
  ...
  const handleCreateRemittance = async () => {
    let prepareData = {
      ...
      files: uploadedDatas.map((item: any) => {
        return { file_id: item?.id, file_type: documentType };
      }),
    };
    ...
    <div className="flex flex-col  justify-start h-[100vh] content-around  items-center drop-shadow-md">
      ...
      {showSelectUsers && (
        <Modal
          ...
          closable
          onCancel={() => setShowSelectUsers(!showSelectUsers)}
        >
          <div className="h-[70vh] flex flex-col justify-between">
            <div>
              ...
              >
                {userOptions.map((item: any) => (
                  <Option
                    key={item?.name || item.first_name}
                    value={(item?.id).toString()}
                  >
                    <div className="flex gap-2">
                      <img
                        src={item.avatar_url || draftProfile}
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-2 mt-2">
                {savedEntity.slice(0, 4).map((entity: any) => {
                  return (
                    <div
                      key={entity?.id}
                      className="bg-white w-full shadow-lg p-2 h-20 hover:bg-slate-100 cursor-pointer"
                      onClick={() => setSelectedUser(entity)}
                    >
                      <div className="flex flex-col relative h-full">
                        <div className="flex flex-row justify-start items-center  gap-4 h-full w-full ml-3 ">
                          <img
                            src={entity?.avatar_url || draftProfile}
            ...
            <div className="flex justify-between mt-4">
              ...
              <Button
                onClick={() => setShowSelectUsers(false)}
                className="!bg-[#2d4da3] text-white"
              >
                Save
              </Button>
            </div>
      ...
      <AddEntityModal
        open={showAddEntity}
        onCancel={() => setShowAddEntity(!showAddEntity)}
      />
          ...
          <div className="bg-[#F6FAFF] mt-2 p-5 w-full md:w-[80vw] rounded-3xl  h-full flex flex-col gap-5 justify-between rounded-b-none">
            <div className="flex flex-col justify-between content-between h-full">
              ...
              <div className="flex flex-col gap-2">
                ...
                <InputSelect
                  inputClassName="!text-[#56AEF5]"
                  label="From Currency"
                  onChangeInput={handleChangeFromAmount}
                  inputValue={fromAmount}
                  inputPlaceHolder="From Amount"
                  selectPlaceHolder="currency"
                  onSelect={handleSelectFromCurrency}
                  selectValue={selectFromCurrency?.id}
                >
                ...
                <InputSelect
                  inputClassName="!text-[#F5AC56]"
                  label="To Currency"
                  onChangeInput={handleChangeToAmount}
                  inputPlaceHolder="From Amount"
                  selectPlaceHolder="currency"
                  inputValue={toAmount}
                  onSelect={handleSelectToCurrency}
                  selectValue={selectToCurrency?.id}
                >
                  {CurrencyListArray?.map((item: any) => (
                    <Select.Option key={item?.id} value={item?.id}>
                      <div className="flex flex-cols gap-2 justify-center content-center self-center">
      ...
      {step == "step2" && (
        <>
          ...
          <div className="bg-[#F6FAFF] mt-2 p-5 w-full md:w-[80vw] rounded-3xl flex flex-col gap-5 rounded-b-none overflow-y-auto h-[calc(100vh_-_220px)] overflow-x-visible ">
            <div className="flex flex-col gap-3">
              <p className="font-thin text-gray-500 flex gap-2">
                Select custormer <MdPeopleAlt className="self-center text-lg" />
              </p>
              {/*  */}
              {!selectedUser && (
                <Button
                  onClick={() => setShowSelectUsers(true)}
                  className="!text-xs !bg-[#2d4da3] text-white !w-[30%] flex gap-2 justify-center"
                >
                  Select
                </Button>
              )}
              {selectedUser && (
                <div className="bg-white h-28 w-full shadow-lg rounded-2xl p-2 max-w-[430px] self-center">
                  <div className="flex flex-col relative h-full">
                    <IoMdClose
                      className="hover:text-red-500 text-gray-500 absolute right-2  top-2 cursor-pointer"
                      onClick={() => setSelectedUser(undefined)}
                    />
                    <div className="flex flex-row justify-start items-center  gap-4 h-full w-full ml-3 ">
                      <img
                        src={selectedUser?.imageUrl || draftProfile}
                      ...
                      <div className="w-full text-right text-gray-500 mr-10">
                        <p className="text-xs text-gray-400"></p>
                        {selectedUser?.rightText}
                      </div>
                    </div>
                  </div>
                </div>
              ...
              <Select
                className="h-12"
                value={transactionPurpose || undefined}
                placeholder="Select Purpose"
                options={purpose}
                onChange={(value) => setTransactionPurpose(value)}
              />
              ...
              <UploadArea
                ...
                department_id={userInfo?.entity?.department_id}
              />
        ...
        <>
          ...
          <div className="bg-[#F6FAFF] mt-2 p-5 w-full md:w-[80vw] rounded-3xl flex flex-col gap-5 rounded-b-none overflow-y-auto h-[calc(100vh_-_220px)] overflow-x-visible ">
            <div className="flex flex-col gap-2">
              ...
              {selectedUser && (
                <div className="bg-white h-28 w-full shadow-lg rounded-2xl p-2 max-w-[430px]">
                  <div className="flex flex-col relative h-full">
                    <div className="flex flex-row justify-start items-center  gap-4 h-full w-full ml-3 ">
                      <img
                        src={selectedUser?.avatar_url || draftProfile}
              ...
              <div className="flex flex-cols gap-2 w-full"></div>
